...
for outFile in sys.argv[2:]:
	...
	while line:
		...
		if (line.startswith("Total Instructions: ")):
			...
			tlb_entries = entriesLine.split()[1]
	...
	print(TLBConfigStr)
	if TLBConfigStr in results:
		results[TLBConfigStr] = (results[TLBConfigStr][0]*ipc,results[TLBConfigStr][1]*mpki)
	else:
		results[TLBConfigStr] = (ipc,mpki)
...
ax1.grid(True)
ax1.set_xlabel("TLBEntries.TLBAssoc.TLBBlockSize")
